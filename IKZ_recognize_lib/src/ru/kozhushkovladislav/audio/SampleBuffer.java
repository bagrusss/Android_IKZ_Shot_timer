...
public class SampleBuffer {
	 ...
	 * @param channels
	 * @param samples
	 *            the buffer can hold how many samples. one sample include all
	 *            channel data. so the buffer capacity is samples * channels.
	 */
	...
	public SampleBuffer(int samplerate, int channels, final ByteBuffer bytes,
			int audioformat) {
		mSampleRate = samplerate;
		mChannels = channels;
	...
	public SampleBuffer(int samplerate, int channels, FloatBuffer buffer) {
		if (buffer.capacity() % channels != 0) {
			throw new IllegalArgumentException(
					"the FloatBuffer capacity must be multiple of channels.");
		}
		mSampleRate = samplerate;
		mChannels = channels;
	...
	public SampleBuffer(int samplerate, int channels, float[] array) {
		if (array.length % channels != 0) {
			throw new IllegalArgumentException(
					"the float array length must be muitipe of channles.");
		}
		mSampleRate = samplerate;
		mChannels = channels;
	...
	public SampleBuffer(int samplerate, int channles, float[] array, int start,
			int floatcount) {
		if (floatcount % channles != 0) {
			throw new IllegalArgumentException(
					"floatcount must be multiple of channels.");
		}
		mSampleRate = samplerate;
		mChannels = channles;
	...
	private void Initialize(int samplerate, int channels,
			final ByteBuffer bytes, int audioformat) {

		if (audioformat != ENCODING_PCM_8BIT
				&& audioformat != ENCODING_PCM_16BIT) {
			throw new IllegalArgumentException(
					"Only support 16bit PCM or 8bit PCM format.");
		}
		int capacity = bytes.limit()
				/ ((audioformat == ENCODING_PCM_8BIT) ? 1 : 2);
		if (capacity % mChannels != 0) {
			throw new IllegalArgumentException(
					"the bytes length must be multipe of channels");
		}
	...
	/**
	 * get a sample. return float[].length = mChannels. and the position
	 * increases mChannels
	 */
	...
	/**
	 * the array that backs the samples buffer. just simple wrap
	 * {@link java.nio.FloatBuffer#array()}
	 * 
	 * @return the array ;
	 */
	...
	/**
	 * proxy {@link java.nio.FloatBuffer#put(java.nio.FloatBuffer)} , and both
	 * src and dest SampleBuffer must have same mSamplerate and mChannels
	 */
	public SampleBuffer put(SampleBuffer src) {
		if (src.mSampleRate != this.mSampleRate
				|| src.mChannels != this.mChannels) {
			throw new IllegalArgumentException(
					"src sample rate or channels is not same as dest.");
		}
	...
	public static SampleBuffer wrap(int samplerate, int channels,
			float[] array, int start, int floatcount) {
